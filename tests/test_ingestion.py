...
from analytics_platform.ingestion import ingest_telemetry, parse_iso_timestamp, to_bool_int
...
class IngestionTests(unittest.TestCase):
    ...
    def test_to_bool_int(self) -> None:
        self.assertEqual(to_bool_int("true"), 1)
        self.assertEqual(to_bool_int("false"), 0)
        self.assertIsNone(to_bool_int("unknown"))
    ...
    def test_ingestion_end_to_end(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(stats.batches_seen, 1)
            self.assertEqual(stats.events_seen, 1)
            self.assertEqual(stats.events_inserted, 1)
            self.assertEqual(stats.bad_batch_json, 0)
            self.assertEqual(stats.bad_event_json, 0)
            self.assertEqual(stats.missing_required, 0)
            self.assertEqual(stats.employees_loaded, 1)
            ...
            self.assertEqual(practice, "Frontend Engineering")
