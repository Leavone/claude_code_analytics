...
def test_build_insights_report(tmp_path) -> None:
    ...
    assert report["overview"]["total_events"] == 4
    assert report["overview"]["total_sessions"] == 2
    assert report["overview"]["total_cost_usd"] == 2.0

    assert len(report["daily_tokens_by_practice"]) >= 2
    assert len(report["peak_usage_hours"]) >= 2
    assert report["tool_performance"][0]["tool_name"] == "Read"
    ...
    assert "predictive_analytics" in report
    assert "history" in report["predictive_analytics"]
    assert "forecast" in report["predictive_analytics"]
